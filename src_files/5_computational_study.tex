...
The solution quality for all four ILS variants (see Fig.~\ref{fig:four_variants}) is compared by running all 27 instances of the
...
(see Table~\ref{tab:chosen_datasets}) are chosen, which results in a total of 37 different configurations. The first insight in the
comparison is presented in Figure~\ref{fig:boxplots_final_comparison}, which shows the boxplots for each classifier variant.
\begin{figure}[!ht]
	\centering
...
\end{figure}
The NoClassifier variant has the lowest median \gls{RPD} and the least number of iterations. The SpeedUp variant is the best performing
classifier variant, with a low \gls{RPD} and significantly the most iterations per instance, which consecutively leads to high percentual
improvement from constructive to best solution. Even though, the Filter variant has a good improvement, the RPD values are the worst as
the constructive solution is worse in comparison to the other variants, as \gls{FN} predictions lead to worse solutions. The Hybrid variant
has decent \gls{RPD} values, but the number of iterations is in comparison to the SpeedUp variant quite low. This topic was already discussed
in last section, and especially this sezup was chosen to differentiate to the SpeedUP variant. The following Table~\ref{tab:mean_final_results}
contains mean performance indicator values per variant. The mean values are shown in Table~\ref{tab:final_mean_comparison} in the appendix.
\begin{table}
	\centering
...
\end{table}
It needs to be noted, that the mean number of iterations is considerably higher than the median. Interestingly,
the difference between the NoClassifier and SpeedUp RPD values is shrunken. Overall the rejection ratio is quite low, leading
to the reversion of only round a third of all iterations. The median values per configuration are shown in Table~\ref{tab:final_mean_comparison} in the appendix. Additionally, the visualizations for each classifiers show, the influence of different model
and datasets:
\begin{itemize}
	\item Filter variant - Figure~\ref{fig:final_results_filter_variant}.
	\item SpeedUp variant - Figure~\ref{fig:final_results_speedup_variant}.
	\item Hybrid variant - Figure~\ref{fig:final_results_hybrid_variant}.
\end{itemize}
However, the differences between single models and datasets for each variant are surprisingly small and do not effect the results
considerably strong. Despite the average results, the results per instance are compared in the following. The best solution
per variant is compared between each other, as well as with the best known solution. In comparison to Table~\ref{tab:bc_results_gendreau} only heuristic approaches are considered and the complete loading constraint set $\mathcal{G}$.
The publisher for  the best known solution(BKS) for the \gendreauDataSetText is indicated by the small letter
in the BKS column. In this comparison all published papers were considered, which use the same loading constraint set $\mathcal{G}$.
\begin{table}
	\centering
	\begin{tabular}{L{6cm}lccc}
		\toprule
		                                                                                                                                      &       & \multicolumn{3}{c}{Timelimit [sec]}                               \\\cline{3-5}
		...
		\cite{bortfeldt_hybrid_2012}\footcite[cf.][p.2253]{bortfeldt_hybrid_2012}\footnote{Start solution time was excluded from time limit.} & c     & 180                                 & 300              & 400      \\
	...
	\caption{Different time limits for \gendreauDataSetText instances dependend of customer number $N$.}
\end{table}
...
From 27 solutions in the literature nine could be improved with the presented alogorithm. Among them SpeedUp and NoClassifier found
each four improvements alone. Generally, the SpeedUp variant performs better for middle to big sized instances in comparison to
the NoClassifier variant. For these variants the high iteration number becomes more valuable. In total has the SpeedUp variant the lowest
mean deviation from the BKS with 0.95\% and NoClassifier with 1.69\% the second best result. The filter variant performs slightly better than the Hybrid variant in this comparison. The performance of the presented algorithm is underlined, as the time limit for obtaining
the best known solutions was for most of the author significantly up to 6 times higher than in this work (see Table~\ref{tab:timeLimit_comparison}).

The mean costs per run and variant are shown in Table~\ref{tab:final_mean_comparison}, which shows, that SpeedUp and NoClassifier variant
have the best results. The same impression confirms, that the SpeedUp variant is slightly better for middle and big sized instances. Filter and Hybrid in average performing not well.