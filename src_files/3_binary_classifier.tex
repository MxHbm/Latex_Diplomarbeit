...
The first approach is to save all routes with exact loading label from an exisiting \gls{3L-CVRP}
...
used to save all routes, which are labeled and found in an 8-hour total timelimit and a timelimit for the \gls{CP} solver of 10 min for each instance.
...
will be classified as unknown, based on the resp. \gls{LFL}.
The \gls{LFL} defines the subset of
loading constraints used. Three \glspl{LFL} are considered in the orginal algorithm, which are shown
in the following:
\begin{itemize}
...
is only relevant for the first two \glspl{LFL}, and the routes are saved as sequences. For the third
group \textit{No Sequence} the found routes are independent from their sequence and an ordered set
of the respective customer ids is saved. \footcites[Retrieved from][]{tamke_repository_2024}[cf.][]{tamke_branch-and-cut_2024}
After the branch-and-cut algorithm the routes are sorted into different sets composed of the \gls{LFL} and \gls{LST}.
All the route sequences found are respected for the train dataset, but for the sets from the third \gls{LFL} group \textit{No Sequence},
two alternative datasets are created, one with sets and one without. For the train dataset with sets, two random permutations are considered from each set
to avoid that the permutations dominate the train dataset number wise. The following table highlights, which routes depending on the
\gls{LFL} and \gls{LST} are considered for the final train datasets.
