...
for the features. Furthermore, \gls{ML} model types can be negatively affected by non-informative features, so features without
an influence to the prediction outcome. This applies to all models, where all predicting values are combined in a statistical formula
...
The feature selection approaches for supervised learning models can be divided in three groups, which are described further afterwards.
\begin{enumerate}
    \item Filter Methods
    \item Wrapper Methods
    \item Embedded approaches
\end{enumerate}
Filter methods are ranking techniques, which are used to filter a subset of features before the model is trained, by analyzing
the correlation between the features and the output variables. By defining a threshold or an explicit number of features, the
final features are selected. It needs to be noted, that inter feature correlation exists and several subsets of features can
generate the same model performance, when high correlated features exist, which is not considered by filter methods.
Furthermore, the type of data of the features and the
output value determines significantly, which method can be applied and is capable of explaining the correlation. The risk
...
exponentially, choosing the right subset is a $NP$-hard problem. Several heuristic methods are used to tackle this task.
Sequential selection algorithms, which can be implemented either forward, starting with 0 features, or backwards, starting with the
maximum number of features and adding resp. removing one feature after each other to find the best combination. Other methods involve
\glspl{GA} or branch-and-bound algorithms. The main difference to filter methods is the focus on the classifiction performance with
different feature subsets. Embedded approaches aim to reduce the computation time needed for the wrapper methods by integrating the
feature selection in the training process. \footcites[cf.][pp.17-21]{chandrashekar_survey_2014}

As this thesis will analyse several different datasets and models, the number of combinations is large, when a perfect feature subset
has to be selected for each dataset model tupel. Therefore, the same subset of features will be used for each model and dataset
by using a filter method approach, to provide comparability. The following algothim flowchart visualizes how the final set
of features is obtained by using several scoring methods as filters and several different train datasets.




To select the best fitting dataset and corresponding feature list for every dataset is iteratively
the optimal feature list selected with FSFS and afterwards all other datasets are labeled
