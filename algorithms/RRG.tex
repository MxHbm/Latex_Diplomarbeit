This Chapter helps understanding the algorithm used for the generation of random routes. The complete algorithm
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        % Nodes
        \node[startstop] (start) {Start RRG ($\alpha$, $\beta$, $\gamma$, $\delta$,$\mathcal{IS}$,)};
        \node[decision, below=of start] (forI) {$i\in\mathcal{IS}$ left?};
        \node[process, left= of forI] (nextI) {Next $i$ \&\\ Save $\mathcal{R}$};
        \node[process, below=of forI] (initI) {$\mathcal{R}\gets\emptyset$\\$n_{\max}\gets \mathrm{MaxCustomers}(i)$\\BreakBool$\gets$False};
        ...
        \node[decision, below=12 mm of whileK] (dup) {$RT\in\mathcal{R}$?};
        ...
        \node[process, above=10mm of feasible] (accept) {$\mathcal{R}\gets \mathcal{R}\cup\{RT\}$\\SuccessBool$\gets$True};
        ...
        \draw[line] (whileK) --node[pos=0.5, right, align=left]{yes \\ $RT\gets \mathrm{RandomTour}(i,n)$} (dup);
        ...
        \node[above left=0mm and -3mm of forI] {\footnotesize For each $i\in\mathcal{IS}$};
    ...
    \end{tikzpicture}
    \caption{Flowchart for random route generation algorithm.}
