\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        ...
        \Procedure{Filter}{Scoring method $\kappa$, Correlation thresholds $\Phi$, Quantile threshold $\epsilon$, Boundary $\mathcal{B}$}
        ...
        \State{Compute mutual correlation matrix $\mathcal{C}_d(\mathcal{F},\mathcal{F})$}
        \ForAll{thresholds $\phi \in \Phi$}
        \ForAll{features $f \in \mathcal{F}$}
        \ForAll{features $f' \in \mathcal{F}\,|\, f \neq f'$}
        \If{$\mathcal{C}_d(f,f') > \phi$}
